...
const Datatable = ({ onShowModal }) => {
  ...
  const totalData = useSelector((state) => state.transaction);
  ...
  const transactionData = useMemo(() => {
    let allData =
      Array.isArray(totalData?.tableData) &&
      totalData.tableData.sort(sortByDateTime);
  ...
  return (
    <>
      ...
      <Table
        ...
        actionItem={(action) => invokeModalAction(action)}
      />
      <Pagination
